...
- Drawing many models impact the programs overall performance because of the thousands of render calls required. You need to bind `VAO`, bind texture, set uniforms, etc. `Rendering` the `vertex data` has `heavy performance overhead` because of this preperations.
...
- While adding the Staggered Grid and fluidSolver over to the main source file, I figured that the program just crashes. Stepping over from a breakpoint set at fluidSolver just crashes the program. Turns out, the issue was just my boundary condition logic because it was out of bounds.
